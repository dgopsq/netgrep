...
Inside a single line longer than 64 KB, netgrep searches a sliding window rather than the whole line, and three things follow. The first two hit both functions, and in both directions, because the window is all the engine is given and neither function can be told otherwise. A match longer than the window, or one that spans the seam between two of them, is missed outright — `matches` answers false and `grep` never yields the line. And `^` and `$` can match at the window's edge, which is not where the line begins or ends: `matches` answers true when no line in the file really begins that way, and `grep` yields the line behind that answer, which does not begin where it claims to. The third is `grep`'s alone. It reports a hit inside such a line more than once, with a line number that gains one at every window slide, so numbers after an over-long line are approximate rather than exact. Two lesser effects: the line a hit carries begins at the window's edge rather than where the line begins, so it is a mid-line fragment; and newline-free input is answered more slowly, because nothing can be searched until the window fills or the download ends. Ordinary log lines, source files and prose are nowhere near this; a minified bundle or a single-line data dump is.
