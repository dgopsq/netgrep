...
netgrep answers whether a pattern occurs in a file and — if you ask — the first line it occurs on, with each match highlighted within it. It does not rank: no match counts, no line numbers, no relevance ordering, so it cannot sort files by how well they match. If you need that, a prebuilt index — [Pagefind](https://pagefind.app/), [Lunr](https://lunrjs.com/), [FlexSearch](https://github.com/nextapps-de/flexsearch) — is the right tool.
