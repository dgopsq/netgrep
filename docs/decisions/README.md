...
| [0025](0025-streaming-grep-over-http.md) | netgrep is streaming grep over HTTP, not client-side site search | Accepted — **amends 0017 and 0023** (the demo's framing), and cites 0002 without touching it; the project is positioned on what it does rather than on what it is not, and **0003 is strengthened rather than pressured** — amended by 0026, which closes the timing half of its open gap: the demo can now show an early answer against a full read |
